...
let mermaidSource = ref<string | null>(null)

onMounted(async () => {
  if (!project.mermaidDiagram) return

  const url = new URL(project.mermaidDiagram, window.location.origin)
  ...
  <div class="container mx-auto px-6 py-3 space-y-8">
    ...
    <div class="grid gap-8 lg:grid-cols-3">
      <div class="lg:col-span-2 space-y-6">
        ...
        <p class="text-text">{{ project.overview }}</p>

        <h3 class="text-lg font-semibold">Architecture</h3>
        <p class="text-text">{{ project.architectureSummary }}</p>
        <img v-if="project.diagram" :src="project.diagram" class="rounded shadow" />
        <Mermaid v-if="mermaidSource" :source="mermaidSource" :title="project.mermaidTitle"></Mermaid>

        <h3 class="text-lg font-semibold">Technical Approach</h3>
        <ul class="list-disc ml-6 space-y-2">
          <li v-for="(t, i) in project.technicalApproach" :key="i">{{ t }}</li>
        </ul>
      ...
      <aside class="space-y-4">
        <div class="bg-card border border-primary rounded-lg p-4 shadow">
          ...
          <p class="font-medium mt-2">{{ project.stack.join(' • ') }}</p>
        </div>
